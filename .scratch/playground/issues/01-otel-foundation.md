...
- [x] Only `cmd` imports `go.opentelemetry.io/otel/sdk/...`; `internal/app` and `internal/domain` import at most the `otel` API (`go list -deps` check in a test or `grep`)
- [x] Every HTTP response from the server carries `X-Trace-ID` (32 hex), equal to the server span's trace id
- [x] An inbound `traceparent` header is continued (response `X-Trace-ID` equals the inbound trace id)
- [x] `agnoforge data …` on a non-2xx prints `trace: <id>` to stderr
